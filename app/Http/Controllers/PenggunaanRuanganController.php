...
class PenggunaanRuanganController extends Controller
{
    ...
    public function __invoke(Request $request)
    {
        ...
        $detail_kegiatan_query = KelasMataKuliah::query()
            ->select("kegiatan_id")
            ->selectRaw("
                json_agg(
                    json_build_object(
                        'tipe', tipe,
                        'mata_kuliah', json_build_object('id', mata_kuliah.id, 'nama', mata_kuliah.nama) ,
        ...
        $jadwals = Jadwal::query()
            ...
            ->withCasts(["detail_penggunaans" => JsonCast::class ])
            ->paginate();
